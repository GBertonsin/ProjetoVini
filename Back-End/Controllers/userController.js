...
exports.register = async (req, res) => {
  const { matricula, senha, email, box, nome, periodo, idCargo } = req.body;
  ...
  try {
    ...
    if (matricula && matriculaStr.length == 7) {
      ...
      return res
        .status(401)
        .json({ message: "Matrícula não preenchida ou inválida" });
    }
    ...
    if (senha && senha.length > 1 && senha.length < 128) {
      ...
      return res
        .status(401)
        .json({ message: "Senha não preenchida ou inválida" });
    }
    ...
    if (email && email.length > 1 && email.length < 128) {
      ...
      return res
        .status(401)
        .json({ message: "E-mail não preenchido ou inválido" });
    }
    ...
    if (nome && nome.length > 1 && nome.length < 128) {
      ...
      return res
        .status(401)
        .json({ message: "Nome não preenchido ou inválido" });
    }
    if (box && box.length > 1 && box.length < 128) {
      ...
      return res
        .status(401)
        .json({ message: "box não preenchido ou inválido" });
    }
    ...
    if (periodo && periodoStr.length <= 2) {
      ...
      return res
        .status(401)
        .json({ message: "Período não preenchido ou inválido" });
    }
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    if (matricula && matriculaStr.length == 7) {
      ...
      return res
        .status(401)
        .json({ message: "Matrícula não preenchida ou inválida", matricula });
    }
    ...
    if (box && box.length > 1 && box.length < 128) {
      ...
      return res
        .status(401)
        .json({ message: "box não preenchida ou inválida" });
    }
